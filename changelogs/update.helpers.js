...
const analyseChangelog = (changelogDataArray, statuses, releaseVersion) => {
  ...
  changelogDataArray.some((line, index) => {
    ...
    if (
      foundHeader &&
      subHeader &&
      statuses.includes(subHeader[1].toLowerCase())
    ) {
...
const buildChangeItem = change => [
  `- _Component [${change.component.replace("-", " ")}]({{ componentPath '@${
    change.component
  }' }})_:`,
  `  ${change.description}`
];
...
const buildChangeItems = changes =>
  changes.reduce(
    (changeItems, change) => [...changeItems, ...buildChangeItem(change)],
    []
  );
...
const buildChanges = (
  ...
  const header = !foundHeader
    ? [`## [${releaseVersion}] - ${today.getFullYear()}-${today.getMonth()}`]
    : [];
  return statuses.reduce((array, status) => {
    const existingChangeItems =
      foundHeader && foundHeader[status]
        ? changelogDataArray.slice(
            foundHeader[status] + 1,
            foundHeader[`${status}End`]
          )
        : [];
